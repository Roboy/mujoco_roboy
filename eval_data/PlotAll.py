...
startsec_pruned = int(input('Start closeup Plots at Second: '))
endsec_pruned = int(input('End closeup Plots at Second: '))
...
def PLOT_SP_LEN(bagdir, PIDvalues, startsec_pruned, endsec_pruned):
...
def PLOT_FORCE(bagdir, PIDvalues, startsec_pruned, endsec_pruned):
    forces=[]
    element =[0,[0]]
    currentdir = os.path.join(bagdir,PIDvalues)
...
for folders in os.listdir(bagdir):
    ...
    PLOT_SP_LEN(bagdir, PIDvalues, startsec_pruned, endsec_pruned)
    PLOT_FORCE(bagdir, PIDvalues, startsec_pruned, endsec_pruned)
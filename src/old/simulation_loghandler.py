...
if doLog:
    ...
    infoFile = open('%s/info' % currLog, 'w')
    recordingFile = ('%s/recording' % currLog)

    fh = logging.FileHandler(recordingFile)
...
def logToLogger(step):
    if step > logStartStep:
        logger.info(str(rospy.Time.now()) + '\n' + ' '.join(map(str, setpoint)) + '\n' + ' '.join(
            map(str, sim.data.ten_length)) + '\n' + ' '.join(map(str, sim.data.actuator_force)) + '\n\n')
...
if __name__ == '__main__':
    ...
    if doLog:
        # Redirect print output to infofile
        infoFile2 = open('%s/info2' % currLog, 'w')
        originalStdout = sys.stdout
        sys.stdout = infoFile2
        printHeader()

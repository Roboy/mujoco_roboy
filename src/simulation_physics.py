...
def publish_tendon_forces(publisher, tendon_forces):
    ...
    for tendon in tendon_names:
        ...
        msg.name = tendon
        msg.position = [0] * len(tendon)
        msg.velocity = [0] * len(tendon)
        msg.effort = [0] * tendon_forces[start_idx:start_idx+len(tendon)]
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if sim_step >= warmup_step:
            ...
            force = (Kp * error + Kd * (error - error_prev) / dt + error_int)*75
        ...
        publish_tendon_forces(pub2, sim.data.actuator_force)
